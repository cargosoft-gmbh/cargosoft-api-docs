...
> existing endpoints and objects will be renamed, moved, restructured, extended, etc. and also expect new endpoints to be
...
First of all each client have to request a bearer Java Web Toke (JWT) from the CargoSoft API. To receive a JWT, username
and password are required. Username and password can only create by CargoSoft.
...
> Attention: Your user may be restricted. The CargoSoft API use user groups and roles to define user access against the
> CargoSoft API endpoint. If your user is restricted or it's not allowed to use an endpoint you needed please contact our
> support.
...
The CargoSoft API works generally with CRUD operations. We attempt to implement each CRUD operation for each functional
group of endpoints if that makes sense. The delete operation is one of the most sensitive function and will not
...
Lookups should be used to search functional objects with given criteria named identifier. The identifier may be
different to each functional group. Which functional identifier are possible take a look to the functional group.

The identifier will be sent in the request body up to the API lookup endpoint. An identifier contains the following
three information:

1. _match_mode_: The match mode is used to compare the criteria against the object in the database. Possible match mods
   are **[EQUALS, CONTAINS, IN, OR, GT, LT, GE, LE]**.
2. _key_: The key of the criteria for the functional object to lookup. Which keys are possible please see to the
...
the test API. The credentials have to request from the CargoSoft support. The received information gives access to a
...
the common possibilities that should be use. The following figure shows the use case to look up a shipment. To keep the
...
CargoSoft has different pools in which status information is stored. These are described in detail in the next
...
into a shipment status if the CargoSoft customer has stored this. Here, for example, it can be defined that the status
ABC of provider XYZ is to be processed as shipment status ETA. In this case ETA would be a freely defined code for the
status. There is therefore no list of valid codes.
...
unless they have not been recorded in CargoSoft. For a complete list of shipment event codes
